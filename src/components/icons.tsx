...
/**
 * The app logo's path data (32x32 grid). Shared by the `LogoIcon` component
 * and `LOGO_ICON_URI`, the image form the host's Apps menu takes.
 */
...
export const LogoIcon = makeIcon('LogoIcon', LOGO_ICON_PATH)

/**
 * The logo as a `data:image/svg+xml` URI — what the host's 'apps-menu' slot
 * accepts for `icon` (an image URI, never a component). The host paints it
 * as a CSS mask in the menu row's text color, so only the SHAPE matters: the
 * fill below is irrelevant to the result, and the icon follows the light and
 * dark theme (and the disabled state) with no work here.
 */
export const LOGO_ICON_URI =
  'data:image/svg+xml,' +
  encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><path fill="currentColor" d="${LOGO_ICON_PATH}"/></svg>`,
  )
